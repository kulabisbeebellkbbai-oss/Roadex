...
describe('ESP32 identity probe', () => {
  ...
  it('disconnects and rejects malformed probe identity', async () => {
    ...
    await expect(probeEsp32Identity({ serial: { async requestPort() { return port; } } }, usbFilters, () => ({
  ...
  it('rejects a serial chooser result outside the project filters before probing', async () => {
    const port = { getInfo: () => ({ usbVendorId: 0xffff, usbProductId: 0xffff }) } as SerialPort;
    const createSession = vi.fn();
    await expect(probeEsp32Identity(
      { serial: { async requestPort() { return port; } } }, usbFilters, createSession,
